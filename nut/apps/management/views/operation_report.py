...
from apps.core.models import  GKUser, Entity, Note, Article, Article_Remark
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if start_date == 'lastmonth':
            ...
            self.start_date, self.end_date = ((datetime.now() - timedelta(days=7)).strftime(TIME_FORMAT)), (
                datetime.now() + timedelta(days=1)).strftime(TIME_FORMAT)
        context['auth_seller_count'] = GKUser.objects.authorized_seller().using('slave').filter(
            date_joined__range=(self.start_date, self.end_date)).count()
        context['auth_author_count'] = GKUser.objects.authorized_author().using('slave').filter(
            date_joined__range=(self.start_date, self.end_date)).count()
        context['entity_count'] = Entity.objects.using('slave').filter(
            created_time__range=(self.start_date, self.end_date)).count()
        context['note_count'] = Note.objects.using('slave').filter(
            post_time__range=(self.start_date, self.end_date)).count()
        entitys = Entity.objects.using('slave').filter(created_time__range=(self.start_date, self.end_date))
        context['entity_count_100'] = entitys.annotate(likes_count=Count('likes')).filter(likes_count__gt=100).count()
        context['entity_count_50'] = entitys.annotate(likes_count=Count('likes')).filter(likes_count__gt=50).count()
        articles = Article.objects.using('slave').filter(created_datetime__range=(self.start_date, self.end_date))
        context['article_count'] = articles.count()
        context['article_read'] = articles.filter(read_count__gt=1000).count()
        context['article_dig'] = articles.annotate(digs_count=Count('digs')).filter(digs_count__gt=10).count()
