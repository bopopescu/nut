...
insert into core_tag (id, tag, tag_hash, status, creator_id, entity_id, created_time, updated_time) 
	select id, tag, tag_hash, status, user_id, entity_id created_time, updated_time from guoku.base_entity_tag 
		where user_id in (select id from core_gkuser) and entity_id ;
```
