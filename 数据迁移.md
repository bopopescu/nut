...
| guoku_category_id  | int(11)      | NO   | MUL | NULL    |                |
+--------------------+--------------+------+-----+---------+----------------+
```
* 迁移用 SQL

```
insert into core_taobao_item_category_mapping select * from guoku.base_taobao_item_category_mapping;
```
...
insert into core_user_follow (follower_id, followee_id, followed_time)
	select follower_id, followee_id, followed_time  from guoku.base_user_follow where follower_id in (select id from core_gkuser) and followee_id in (select id from core_gkuser );
```
