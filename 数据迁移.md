...
insert into core_user_profile
	select p.id, p.user_id, p.nickname , p.location, p.city, p.gender, p.bio, p.website, p.email_verified
...
insert into core_selection_entity (entity_id, pub_time) 
	select id, created_time from core_entity;
update core_selection_entity is_published = 1;
```
